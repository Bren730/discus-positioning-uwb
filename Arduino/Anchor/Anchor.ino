...
const uint8_t PIN_IRQ = 3; // irq pin, set to 3 for Trinket
...
void setup() {
  ...
  Serial.begin(115200);
  delay(1000);
  Serial.println("Hello! My i2c address is " + String(address));
  Serial.println();
...
void onRequest(){
  ...
  uint16_t distanceMm = round(distance * 1000);
  uint8_t high = (byte) ((distanceMm >> 8) & 0xff);
  uint8_t low = (byte) (distanceMm & 0xff);
  byte byteArray[2] = {high, low};

  // Start transmission to host (address 0)
